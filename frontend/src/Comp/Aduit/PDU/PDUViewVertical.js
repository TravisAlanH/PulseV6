...
export default function PDUViewVertical({ CabinetSide, DepthPosition }) {
  ...
  console.log(CabinetSide);

  let Label = `${CabinetSide.replace("Side", "")} - ${DepthPosition.slice(0, 1)}`;

  return (
    <div className="w-[3rem] flex flex-col justify-start items-center border-2 sticky h-[34rem] top-[5rem]">
      ...
      <div className="h-[3rem] flex flex-row justify-center items-center border-t-2 w-full">1</div>
    </div>
  );
}
